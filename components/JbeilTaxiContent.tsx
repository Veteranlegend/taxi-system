...
export default function JbeilTaxiContent() {
  ...
  const whatsappMessage = useMemo(() => {
    const base = t.jbeilGeneralMessage;
  ...
  }, [t.jbeilGeneralMessage, destinationCity, note, pickupLink, isArabic]);
  ...
  return (
    <LandingPageLayout
      title={t.jbeilPageTitle}
      description={t.jbeilPageDescription}
      ...
      <div className="space-y-10">
        ...
        <div className="grid gap-5 lg:grid-cols-2">
          ...
          <div className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-[0_4px_20px_rgba(15,23,42,0.06)] sm:p-7">
            <div className="flex flex-col gap-5">
              ...
              {/* WhatsApp CTA */}
              <WhatsAppCTAButton href={whatsappUrl} label={t.jbeilBookNow} />
            </div>
          </div>
        </div>
        ...
        <div>
          <p className="mb-5 text-xs font-semibold uppercase tracking-[0.22em] text-slate-500">
            {t.jbeilBenefitsLabel}
          </p>
          <div className="grid gap-4 sm:grid-cols-2">
            {t.jbeilBenefits.map(({ title, body }) => (
        ...
        <div>
          ...
          <div className="space-y-3">
            {t.jbeilRoutes.map(({ from, to, message }) => (
        ...
        <div className="rounded-[1.75rem] border border-emerald-100 bg-emerald-50 p-7 text-center">
          <p className="text-lg font-bold text-slate-900">{t.jbeilCtaTitle}</p>
          <p className="mt-2 text-sm leading-6 text-slate-500">{t.jbeilCtaBody}</p>
          <div className="mt-5">
            <WhatsAppCTAButton href={whatsappUrl} label={t.jbeilBookNow} />
          </div>
        </div>
